...
def main():
    ...
    while True:
        ...
        llm_host.launch_request(
            ...
            50,  # num_tokens
            2000,  # max_num_tokens
            [i for i in range(50)],  # token_ids
            True,  # set_routing
            [1, 3, 4, 0],  # server_ids
            [0, 2, 3, -1],  # start_layer_ids
            [2, 3, 4, -1],  # end_layer_ids
        )
        llm_host.launch_request(
            "prompt",  # request_type
            2,  # request_id
            30,  # num_tokens
            2000,  # max_num_tokens
            [i for i in range(30)],  # token_ids
            True,  # set_routing
            [2, 1, 3, 4, 0],  # server_ids
            [0, 1, 2, 3, -1],  # start_layer_ids
            [1, 2, 3, 4, -1],  # end_layer_ids
        )
        end = time.time()
